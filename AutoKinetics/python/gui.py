...
from PyQt6.QtCore import Qt, QPointF, QRectF, QLineF, pyqtSignal, QThread
...
class SimulationThread(QThread):
    ...
    def __init__(self, kin_file_path, sim_time, temp_k, plot_paths):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            command = [
                ...
                "--p_conc", self.plot_paths["concentration"],
                "--p_zero", self.plot_paths["zero_order"],
                "--p_first", self.plot_paths["first_order"],
                "--p_second", self.plot_paths["second_order"],
            ]
...
class PlotDialog(QDialog):
    ...
    def create_overview_tab(self):
        ...
        summary_text_edit.setText(summary)
        summary_text_edit.setMaximumHeight(150)
        
        image_label = QLabel("Plot wird geladen...")
        image_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        pixmap = QPixmap(self.results["plot_files"]["concentration"])
        ...
        layout.addWidget(QLabel("<b>Analyse-Zusammenfassung:</b>"))
        layout.addWidget(summary_text_edit)
        layout.addWidget(QLabel("<b>Konzentrationsverlauf:</b>"))
        layout.addWidget(image_label)
        
        self.tabs.addTab(tab, "Übersicht & Konzentrationen")
    ...
    def create_analysis_tabs(self):
        ...
        for order, title in zip(orders, titles):
            path = self.results["plot_files"].get(order)
            if path:
                tab = QWidget()
                layout = QVBoxLayout(tab)
                image_label = QLabel()
    ...
    def build_summary_text(self):
        ...
        if not analysis:
            return "Keine Analyseergebnisse für Reaktanten gefunden."
            
        for rate_label, data in analysis.items():
            ...
            summary += (f"Reaktion '{rate_label}':\n"
                        f"  - Analysierter Reaktant: {data['analyzed_reactant']}\n"
                        f"  - Beste Passung: {order} (R² = {data['r_squared']:.4f})\n"
                        f"  - Berechnete Rate k = {data['calculated_k']:.4f} {k_unit}\n\n")
        return summary
...
class SpeciesItem(QGraphicsTextItem):
    ...
    def to_dict(self):
        return {key: getattr(self, key) for key in [
            'formula', 'smiles', 'molar_mass', 'concentration', 'start_concentration', 'delta_hf', 's0', 
...
class ArrowItem(QGraphicsPathItem):
    def __init__(self, start_item, end_item, rate_constant="k", parent=None):
        ...
        self.rate_expression = "k = A * (T/T_ref)^n * exp(-Ea/RT)"; self.update_position()
    def to_dict(self): return {key: getattr(self, key) for key in ['rate_constant', 'arrow_type', 'reaction_string', 'reaction_order', 'temperature_min', 'temperature_max', 'pressure', 'arrhenius_A', 'temperature_exponent_n', 'activation_energy_Ea', 'rate_expression']}
...
class PropertiesPanel(QWidget):
    def __init__(self): super().__init__(); self.form_layout = QFormLayout(); self.setLayout(self.form_layout); self.current_item = None; self.show_welcome_message()
    def show_properties(self, item):
        self.clear_layout(); self.current_item = item
        if isinstance(item, SpeciesItem): self.show_species_properties(item)
        elif isinstance(item, ArrowItem): self.show_arrow_properties(item)
        elif isinstance(item, GroupItem): self.form_layout.addRow(QLabel(f"<h3>Gruppe: {item.title}</h3>"))
    def show_arrow_properties(self, item):
        ...
        for group_name, props in groups.items():
            ...
            for label, attr in props:
                widget = QLineEdit(str(getattr(item, attr))); widget.textChanged.connect(lambda text, a=attr: self.update_property(a, text)); self.form_layout.addRow(label + ":", widget)
    ...
    def show_species_properties(self, item):
        groups = {"Grundlagen": [("Name", "name"), ("Summenformel", "formula"), ("SMILES", "smiles")], "Kinetik & Startwerte": [("Startkonzentration (mol/L)", "start_concentration"), ("Akt. Konzentration (mol/L)", "concentration")], "Rolle in Reaktion": [("Rolle", "role")], "Thermodynamik": [("Molare Masse (g/mol)", "molar_mass"), ("ΔHf (kJ/mol)", "delta_hf"), ("S0 (J/mol*K)", "s0")], "Optionale Eigenschaften": [("Löslichkeit", "solubility"), ("Stabilität", "stability"), ("Nebenreaktionen", "side_reactions_tendency")]}
        for group_name, props in groups.items():
            self.form_layout.addRow(QLabel(f"<b>{group_name}</b>"))
            for label, attr in props:
                if attr == "role":
                    widget = QComboBox(); widget.addItems(["Reactant/Product", "Catalyst", "Inhibitor"]); widget.setCurrentText(item.role); widget.currentTextChanged.connect(lambda text, a=attr: self.update_property(a, text))
                else: widget = QLineEdit(item.toPlainText() if attr == "name" else str(getattr(item, attr))); widget.textChanged.connect(lambda text, a=attr: self.update_property(a, text))
                self.form_layout.addRow(label + ":", widget)
        checkbox = QCheckBox(); checkbox.setChecked(item.is_intermediate); checkbox.stateChanged.connect(lambda state: self.update_property("is_intermediate", state == Qt.CheckState.Checked.value)); self.form_layout.addRow("Ist Zwischenprodukt:", checkbox)
    def update_property(self, attr, value):
        if not self.current_item: return
        try:
            if isinstance(value, bool): setattr(self.current_item, attr, value)
            elif attr == "name": self.current_item.setPlainText(value)
...
class GraphicsScene(QGraphicsScene):
    ...
    def serialize(self):
        species, arrows, groups = [i for i in self.items() if isinstance(i, SpeciesItem)], [i for i in self.items() if isinstance(i, ArrowItem)], [i for i in self.items() if isinstance(i, GroupItem)]
        s_map, g_map = {item: i for i, item in enumerate(species)}, {item: f"group_{i}" for i, item in enumerate(groups)}; node_map = s_map | g_map
        species_data = [item.to_dict() for item in species]; group_data = [{'id': g_map[g], 'title': g.title, 'items': [s_map[s] for s in g.contained_items if s in s_map]} for g in groups]
        arrow_data = [arrow.to_dict() | {'start_id': node_map.get(arrow.start_item), 'end_id': node_map.get(arrow.end_item)} for arrow in arrows]
    ...
    def deserialize(self, data):
        ...
        species_items = [SpeciesItem.from_dict(sd) for sd in data['species']]
        for item in species_items: self.addItem(item)
        group_items = {}
        if 'groups' in data:
            for group_data in data.get('groups', []):
                contained = [species_items[i] for i in group_data['items'] if 0 <= i < len(species_items)]
                group = GroupItem(contained, group_data['title']); group_items[group_data['id']] = group; self.addItem(group)
        node_map = {i: item for i, item in enumerate(species_items)} | group_items
        if 'arrows' in data:
            for arrow_data in data['arrows']:
                start_node, end_node = node_map.get(arrow_data['start_id']), node_map.get(arrow_data['end_id'])
                if start_node and end_node:
                    ...
                    arrow.set_arrow_type(arrow_data.get('arrow_type', 'Forward')); self.addItem(arrow)
    ...
    def delete_selected_items(self):
        ...
        if not items: return
        all_items_to_delete = set(items)
        for item in items:
            if isinstance(item, (SpeciesItem, GroupItem)): all_items_to_delete.update(item.arrows)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.plot_output_paths = { "concentration": str(Path(tempfile.gettempdir()) / "sim_plot_conc.png"), "zero_order": str(Path(tempfile.gettempdir()) / "sim_plot_zero.png"), "first_order": str(Path(tempfile.gettempdir()) / "sim_plot_first.png"), "second_order": str(Path(tempfile.gettempdir()) / "sim_plot_second.png")}
    ...
    def create_properties_dock(self):
        dock = QDockWidget("Eigenschaften"); self.properties_panel = PropertiesPanel(); dock.setWidget(self.properties_panel); self.addDockWidget(Qt.DockWidgetArea.RightDockWidgetArea, dock)
    def handle_start_simulation(self):
        try: sim_time = float(self.sim_time_edit.text()); temp_k = float(self.sim_temp_edit.text())
        except ValueError: QMessageBox.critical(self, "Fehler", "Ungültige Eingabe für Simulationsparameter."); return
        with tempfile.NamedTemporaryFile(mode='w', delete=False, suffix=".kin", encoding='utf-8') as tmp_file:
            json.dump(self.scene.serialize(), tmp_file); self.temp_kin_path = tmp_file.name
        self.statusBar().showMessage("Simulation läuft..."); self.start_simulation_action.setEnabled(False)
        self.sim_thread = SimulationThread(self.temp_kin_path, sim_time, temp_k, self.plot_output_paths)
    ...
    def handle_create_group(self):
        ...
        if ok and title: cmd = AddCommand(GroupItem(selected, title), self.scene, "Gruppe hinzugefügt"); self.undo_stack.push(cmd)
    def handle_open(self):
        ...
        if path:
            ...
            except Exception as e: self.statusBar().showMessage(f"Fehler beim Laden: {e}", 5000)
    ...
    def on_selection_changed(self):
        ...
        if len(items) == 1: self.properties_panel.show_properties(items[0])
        else: self.properties_panel.show_welcome_message()

# =============================================================================
# 5. ANWENDUNGS-STARTPUNKT
# =============================================================================
